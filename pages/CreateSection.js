// CreateSection.js - Page Object Model for full Create Course flow
...
class CreateSection {
  constructor(page) {
    ...
    // Track if Preview Report has already been handled to avoid opening twice
  ...
  async fillCourseDetails() {
    ...
    const titleInput = this.page
      ...
      .first();
    // Keep title short (2 words) to avoid long URLs; append short suffix for uniqueness
    const uniqueTitle = `PW Test-${Date.now().toString().slice(-4)}`;
    ...
    // URL - Only append the slug to the default URL
    ...
    await urlInput.fill(baseUrl + 'playwright-automation-testing');
    ...
    for (const [topicIndex, topic] of sectionData.topics.entries()) {
      ...
      try {
        if (isQuiz) {
          ...
          // After video, check if Quiz was automatically added and if it's the last content
          // The addQuizTopicAfterVideo is called automatically, which will throw PREVIEW_REPORT_CLICKED_STOP_TOPICS
          // So we don't need to check here - the error handler will catch it
        }
      } catch (error) {
        // If Preview Report was clicked, stop adding more topics
        if (error instanceof Error && error.message === 'PREVIEW_REPORT_CLICKED_STOP_TOPICS') {
          console.log('    ⚠ Preview Report was clicked - stopping topic addition');
          break; // Break out of the topic loop
        }
        // Otherwise, re-throw the error
        throw error;
      }
   ...
   * NOTE: Preview Report button click is skipped - method does nothing
   */
  async handlePreviewReportAfterQuiz() {
    console.log('      - Skipping Preview Report button click (as requested)');
    return;
  }
  ...
  async addTopicWithVideo(sectionContainer, topicTitle, videoUrl) {
    ...
    if (saveButtonVisible) {
      ...
      // After waiting, automatically add quiz topic
      try {
        await this.addQuizTopicAfterVideo(sectionContainer);
      } catch (error) {
        ...
        console.log(`      ⚠ Error adding quiz topic after video: ${error instanceof Error ? error.message : error}`);
      }
    } else {
  ...
  async addTopicWithQuizDetails(sectionContainer, topicTitle, questions, quizDetails) {
    ...
    // Step 5.5: Enable AI-Based Assessment Report if requested
    if (quizDetails && quizDetails.enableAIReport) {
      console.log('      - Enabling AI-Based Assessment Report...');
      ...
      const checkboxVisible = await generateReportCheckbox.isVisible({ timeout: 5000 }).catch(() => false);
      if (checkboxVisible) {
        ...
        // Step 5.6: Fill in the Report Preference (Prompt) textarea
        console.log('      - Filling Report Preference prompt...');
        const reportPrompt = 'Evaluate the student\'s quiz performance based on accuracy, time taken, and consistency. Highlight strengths and areas that need improvement in a short report.';
        ...
        const textareaVisible = await reportPreferenceTextarea.isVisible({ timeout: 5000 }).catch(() => false);
        if (textareaVisible) {
          ...
          // After adding prompt, do nothing further (requested to skip follow-up actions)
        } else {
          console.log('      ⚠ Report Preference textarea not found, skipping prompt fill...');
        }
      } else {
  ...
  async addQuizQuestion(question, isFirstQuestion) {
    // Step 0: Check if this question already exists to prevent duplicates
    const allQuestionInputs = this.page.getByPlaceholder('Let\'s ask a question');
    const questionCount = await allQuestionInputs.count().catch(() => 0);
    
    let questionExists = false;
    for (let i = 0; i < questionCount; i++) {
      const existingQuestionInput = allQuestionInputs.nth(i);
      const existingText = await existingQuestionInput.inputValue().catch(() => '');
      if (existingText.trim() === question.questionText.trim()) {
        questionExists = true;
        console.log(`        ⚠ Question already exists, skipping duplicate: "${question.questionText.substring(0, 50)}..."`);
        break;
      }
    }
    
    if (questionExists) {
      return; // Skip adding duplicate question
    }
    ...
    // Step 2: Find and fill the question text input (use last to get the newly added one)
  ...
  /**
   * Click Save button after Preview Report is closed
   * This is called immediately after closing Preview Report
   */
  async clickSaveAfterPreviewReport() {
    try {
      // Wait for overlays to clear
      await this.ensurePreviewClosed();
      
      // Wait for loading overlays to disappear
      const saveLoadingOverlay = this.page.locator('.ngx-overlay.loading-foreground');
      await saveLoadingOverlay.waitFor({ state: 'hidden', timeout: 10000 }).catch(() => {});
      await this.page.waitForTimeout(1000);
      
      // Find Save button - look for quiz Save button specifically
      const saveButton = this.page
        .locator('button.quiz-save-btn')
        .or(this.page.locator('button:has-text("Save"):not(:has-text("Save as"))'))
        .or(this.page.getByRole('button', { name: /^Save$/i, exact: true }))
        .first();
      
      const saveButtonVisible = await saveButton.isVisible({ timeout: 10000 }).catch(() => false);
      if (saveButtonVisible) {
        ...
        try {
          ...
          console.log('      ✓ Save button clicked after Preview Report');
        } catch (clickError) {
          console.log('      ⚠ Normal Save click failed, trying force click...');
          await saveButton.click({ force: true, timeout: 10000 });
          console.log('      ✓ Save button clicked after Preview Report (force click)');
        }
        ...
        console.log('      ⚠ Save button not found after Preview Report');
        return false;
      }
    } catch (error) {
      console.log(`      ⚠ Error clicking Save after Preview Report: ${error instanceof Error ? error.message : error}`);
      return false;
    }
  }
  ...
  async saveCourseAndVerify() {
    ...
    // Note: If Save was already clicked after Preview Report (via clickSaveAfterPreviewReport),
    // this method will still try to save again. That's okay as it ensures the save happens.

    // CRITICAL: Ensure all modals/overlays are closed before attempting to save
    await this.ensurePreviewClosed();

    // Wait for any loading overlays to disappear
    const initialLoadingOverlay = this.page.locator('.ngx-overlay.loading-foreground');
    await initialLoadingOverlay.waitFor({ state: 'hidden', timeout: 5000 }).catch(() => {});
    
    // Wait for modal containers to be hidden/detached
    const modalContainer = this.page.locator('nz-modal-container, .cdk-overlay-container');
    await modalContainer.waitFor({ state: 'hidden', timeout: 5000 }).catch(() => {});
    ...
    // Wait for Save button to be visible and actionable
    await saveButton.waitFor({ state: 'visible', timeout: 10000 });
    ...
    if (!isEnabled) {
      console.log('⚠ Save button disabled, waiting...');
      await this.page.waitForTimeout(1500);
    ...
    const blockingOverlay = this.page.locator('.ngx-overlay, nz-modal-container, .cdk-overlay-backdrop').first();
    const isBlocking = await blockingOverlay.isVisible().catch(() => false);
    if (isBlocking) {
      console.log('⚠ Overlay still present, waiting for it to disappear...');
      await blockingOverlay.waitFor({ state: 'hidden', timeout: 5000 }).catch(() => {});
    }

    // Wait for any loading overlays to clear
    const saveLoadingOverlay = this.page.locator('.ngx-overlay.loading-foreground');
    await saveLoadingOverlay.waitFor({ state: 'hidden', timeout: 5000 }).catch(() => {});
    
    // Wait a bit for UI to stabilize (collapsed topic headers, etc.)
    await this.page.waitForTimeout(1000);
    
    // Scroll to ensure Save button is in view and not blocked
    await saveButton.scrollIntoViewIfNeeded();
    await saveButton.evaluate(el => el.scrollIntoView({ block: 'center', inline: 'center' }));
    await this.page.waitForTimeout(500);
    
    // Try normal click first, if it fails due to blocking elements, use force click
    try {
      await saveButton.click({ timeout: 10000 });
      console.log('✓ Save button clicked');
    } catch (clickError) {
      // If normal click fails due to blocking UI elements, try force click
      console.log('⚠ Normal Save click failed, trying force click...');
      await saveButton.click({ force: true, timeout: 10000 });
      console.log('✓ Save button clicked (force click)');
    }

    // Wait briefly for save to process
    await this.page.waitForTimeout(2000);
    console.log('✅ Save flow completed');
  }

  // ========================================================================
  // OVERLAY CLEANUP (Preview report modal/iframe blockers)
  // ========================================================================
  async waitForPreviewOverlayToDisappear() {
    const overlays = [
      this.page.locator('.cdk-overlay-container iframe'),
      this.page.locator('.cdk-overlay-container .cdk-overlay-backdrop'),
      this.page.locator('.cdk-overlay-container .cdk-overlay-pane'),
      this.page.locator('.ngx-overlay'),
    ];
  ...
  // Ensure preview modal/iframe is fully closed
  async ensurePreviewClosed() {
    // First attempt: wait for overlays to clear
    await this.waitForPreviewOverlayToDisappear();

    // If still present, try clicking backdrop and waiting again
    const backdrop = this.page.locator('.cdk-overlay-backdrop');
    const overlayIframe = this.page.locator('.cdk-overlay-container iframe');

    const overlayVisible = await Promise.race([
      overlayIframe.isVisible().catch(() => false),
    ...
    // Final fallback: extra escapes if still present
    const stillVisible = await Promise.race([
      overlayIframe.isVisible().catch(() => false),
      backdrop.isVisible().catch(() => false),
    ]);
    if (stillVisible) {
  ...
  // ===========================================================================
  // FINAL STEP: CONTINUE, CERTIFICATE & PUBLISH
  // ===========================================================================

  async continueAndPublishWithCertificate() {
    console.log('➡ Continuing to Publish with certificate...');

    // Guard: ensure page is still open
    const pageAlive = await this.page.evaluate(() => document.readyState).catch(() => null);
    if (!pageAlive) {
      console.log('⚠ Page/context closed – skipping Continue/Publish');
      return;
    }
    ...
    const continueButton = this.page
      .getByRole('button', { name: /^continue$/i, exact: false })
    ...
    if (continueVisible) {
      ...
      const continueLoadingOverlay = this.page.locator('.ngx-overlay.loading-foreground');
      await continueLoadingOverlay.waitFor({ state: 'hidden', timeout: 5000 }).catch(() => {});
      
      await continueButton.waitFor({ state: 'visible', timeout: 10000 });
      await continueButton.scrollIntoViewIfNeeded();
      await continueButton.evaluate(el => el.scrollIntoView({ block: 'center', inline: 'center' }));
      await this.page.waitForTimeout(500);
      ...
      try {
        ...
        console.log('⚠ Normal Continue click failed, trying force click...');
        await continueButton.click({ force: true, timeout: 10000 });
        console.log('✓ Continue clicked (force click)');
      }
      
      // Wait for page to load after Continue (might navigate to preview/publish page)
      await this.page.waitForLoadState('load', { timeout: 15000 }).catch(() => {});
      await this.page.waitForTimeout(1500);
    } else {
      console.log('⚠ Continue button not found');
      return; // Can't proceed without Continue button
    }

    // Step 2: Check "Issue a certificate for this course" checkbox
    console.log('➡ Checking "Issue a certificate for this course" checkbox...');
    
    // Wait for page to be ready after Continue click
    await this.page.waitForTimeout(2000);
    
    // Try multiple strategies to find the certificate checkbox
    let certificateCheckbox = null;
    let checkboxFound = false;
    
    // Strategy 1: Find by label text (most reliable)
    try {
      const labelWithText = this.page.locator('label:has-text("Issue a certificate")').first();
      const labelVisible = await labelWithText.isVisible({ timeout: 5000 }).catch(() => false);
      if (labelVisible) {
        certificateCheckbox = labelWithText.locator('input[type="checkbox"]').first();
        const checkboxVisible = await certificateCheckbox.isVisible({ timeout: 5000 }).catch(() => false);
        if (checkboxVisible) {
          checkboxFound = true;
          console.log('      ✓ Found certificate checkbox via label');
        }
      }
    } catch (e) {
      // Continue to next strategy
    ...
    // Strategy 2: Find checkbox near "Issue a certificate" text
    if (!checkboxFound) {
      try {
        const certificateText = this.page.getByText(/Issue a certificate/i, { exact: false }).first();
        const textVisible = await certificateText.isVisible({ timeout: 5000 }).catch(() => false);
        if (textVisible) {
          // Find checkbox in the same container or nearby
          certificateCheckbox = certificateText.locator('xpath=following::input[@type="checkbox"][1]')
            .or(certificateText.locator('xpath=preceding::input[@type="checkbox"][1]'))
            .or(certificateText.locator('xpath=ancestor::*[1]//input[@type="checkbox"]'))
            .first();
          const checkboxVisible = await certificateCheckbox.isVisible({ timeout: 5000 }).catch(() => false);
          if (checkboxVisible) {
            checkboxFound = true;
            console.log('      ✓ Found certificate checkbox near text');
          }
        }
      } catch (e) {
        // Continue to next strategy
    ...
    // Strategy 3: Find by Ant Design checkbox class
    if (!checkboxFound) {
      try {
        const antCheckboxes = this.page.locator('input.ant-checkbox-input[type="checkbox"]');
        const checkboxCount = await antCheckboxes.count().catch(() => 0);
        if (checkboxCount > 0) {
          // Try to find the one near "certificate" text
          for (let i = 0; i < checkboxCount; i++) {
            const checkbox = antCheckboxes.nth(i);
            const container = checkbox.locator('xpath=ancestor::*[contains(., "certificate") or contains(., "Certificate")][1]');
            const containerCount = await container.count().catch(() => 0);
            if (containerCount > 0) {
              certificateCheckbox = checkbox;
              checkboxFound = true;
              console.log('      ✓ Found certificate checkbox by Ant Design class');
              break;
            }
          }
        }
    ...
    // Strategy 4: Find all checkboxes and check the one that's not checked yet (last resort)
    if (!checkboxFound) {
      try {
        const allCheckboxes = this.page.locator('input[type="checkbox"]');
        const allCount = await allCheckboxes.count().catch(() => 0);
        console.log(`      - Found ${allCount} checkboxes on page, searching for certificate checkbox...`);
        
        for (let i = 0; i < allCount; i++) {
          const checkbox = allCheckboxes.nth(i);
          const isVisible = await checkbox.isVisible({ timeout: 1000 }).catch(() => false);
          if (isVisible) {
            // Check if this checkbox is near certificate text
            const nearbyText = checkbox.locator('xpath=ancestor::*[contains(., "certificate") or contains(., "Certificate")][1]');
            const hasText = await nearbyText.count().catch(() => 0);
            if (hasText > 0) {
              certificateCheckbox = checkbox;
              checkboxFound = true;
              console.log('      ✓ Found certificate checkbox by searching all checkboxes');
              break;
            }
          }
        }
      } catch (e) {
        console.log(`      ⚠ Error searching for checkbox: ${e instanceof Error ? e.message : e}`);
      }
    }
    
    // Now check the checkbox if found
    if (checkboxFound && certificateCheckbox) {
      try {
        await certificateCheckbox.waitFor({ state: 'visible', timeout: 10000 });
        await certificateCheckbox.scrollIntoViewIfNeeded();
        await this.page.waitForTimeout(500);
        
        const checked = await certificateCheckbox.isChecked().catch(() => false);
        if (!checked) {
          // Try checking directly
          try {
            await certificateCheckbox.check({ timeout: 10000 });
            console.log('      ✓ Certificate checkbox checked');
          } catch (checkError) {
            // If check() fails, try clicking the checkbox or its label
            console.log('      ⚠ Check() failed, trying click...');
            try {
              await certificateCheckbox.click({ timeout: 10000 });
              console.log('      ✓ Certificate checkbox clicked');
            } catch (clickError) {
              // Try clicking the label or wrapper
              const label = certificateCheckbox.locator('xpath=ancestor::label[1]').or(
                certificateCheckbox.locator('xpath=following::label[1]')
              ).first();
              const labelVisible = await label.isVisible({ timeout: 2000 }).catch(() => false);
              if (labelVisible) {
                await label.click({ timeout: 5000 });
                console.log('      ✓ Certificate checkbox checked via label click');
              } else {
                // Force click as last resort
                await certificateCheckbox.click({ force: true, timeout: 10000 });
                console.log('      ✓ Certificate checkbox checked via force click');
              }
            }
          }
          await this.page.waitForTimeout(500);
          ...
          // Verify it's checked
          const nowChecked = await certificateCheckbox.isChecked().catch(() => false);
          if (nowChecked) {
            console.log('      ✓ Certificate checkbox verified as checked');
          } else {
            console.log('      ⚠ Certificate checkbox may not be checked - verification failed');
          }
        } else {
          console.log('      ✓ Certificate checkbox already checked');
        }
      } catch (error) {
        console.log(`      ⚠ Error checking certificate checkbox: ${error instanceof Error ? error.message : error}`);
      }
    } else {
      console.log('      ⚠ Certificate checkbox not found - continuing anyway');
    }
    ...
    const publishButton = this.page
      .getByRole('button', { name: /Publish/i, exact: false })
      .or(this.page.locator('button:has-text("Publish")'))
      .or(this.page.locator('button:has-text("publish")'))
      .first();
    ...
    if (publishVisible) {
      // Wait for any loading overlays to clear
      const publishLoadingOverlay = this.page.locator('.ngx-overlay.loading-foreground');
      await publishLoadingOverlay.waitFor({ state: 'hidden', timeout: 5000 }).catch(() => {});
      ...
      try {
        ...
        console.log('⚠ Normal Publish click failed, trying force click...');
        await publishButton.click({ force: true, timeout: 10000 });
        console.log('✓ Publish button clicked (force click)');
      }
  ...
  /**
   * Collapse quiz dropdown if expanded to avoid blocking section add
   */
  async collapseQuizDropdownIfExpanded() {
    try {
      // Target any expanded collapse header containing "Content type: Quiz"
      const expandedQuizHeader = this.page.locator('button[aria-expanded="true"]').filter({
        hasText: /Content type:\s*Quiz/i,
      }).first()
        .or(this.page.locator('div[role="button"][aria-expanded="true"]').filter({
          hasText: /Content type:\s*Quiz/i,
        }).first());

      const visible = await expandedQuizHeader.isVisible({ timeout: 3000 }).catch(() => false);
      if (visible) {
        await expandedQuizHeader.scrollIntoViewIfNeeded();
        await this.page.waitForTimeout(300);
        try {
          await expandedQuizHeader.click({ timeout: 3000 });
          console.log('      ✓ Collapsed quiz dropdown');
        } catch (err) {
          console.log('      ⚠ Normal collapse click failed, trying force click...');
          await expandedQuizHeader.click({ force: true, timeout: 3000 });
          console.log('      ✓ Collapsed quiz dropdown (force)');
        }
        await this.page.waitForTimeout(500);
      } else {
        console.log('      - No expanded quiz dropdown to collapse');
      }
    } catch (error) {
      console.log(`      ⚠ Could not collapse quiz dropdown: ${error instanceof Error ? error.message : error}`);
    }
  }

  /**
   * Close the Preview Report modal using multiple strategies
   */
  async closePreviewModal() {
    try {
      // Wait for loading overlay to disappear before attempting to close
      const loadingOverlay = this.page.locator('.ngx-overlay.loading-foreground');
      await loadingOverlay.waitFor({ state: 'hidden', timeout: 5000 }).catch(() => {});
      await this.page.waitForTimeout(300);

      // If a modal is present, note it
      const modalWrap = this.page.locator('.ant-modal-wrap, .cdk-overlay-pane, nz-modal-container').first();
      await modalWrap.waitFor({ state: 'visible', timeout: 2000 }).catch(() => {});

      // Soft attempt: send one Escape first
      await this.page.keyboard.press('Escape').catch(() => {});
      await this.page.waitForTimeout(150);

      const primaryCloseButton = this.page.locator('button:has-text("Close")')
        .or(this.page.getByRole('button', { name: /^Close$/i, exact: true }))
        .or(this.page.locator('button[aria-label*="close" i]'))
        .or(this.page.locator('.ant-modal-close, .ant-modal-close-x'))
        .first();
      const overlayCloseButton = this.page.locator('.cdk-overlay-container button:has-text("Close")').first();
      const backdrop = this.page.locator('.cdk-overlay-backdrop').first();

      const tryClickClose = async (btn) => {
        ...
        try {
          ...
          console.log('      ✓ Closed Preview Report');
          return true;
        } catch {
          console.log('      ⚠ Normal close click failed, trying force click...');
          await btn.click({ force: true, timeout: 3000 });
          console.log('      ✓ Closed Preview Report (force click)');
          return true;
        }
      };

      let closed = false;
      if (await primaryCloseButton.isVisible({ timeout: 1500 }).catch(() => false)) {
        closed = await tryClickClose(primaryCloseButton).catch(() => false);
      }
      if (!closed && await overlayCloseButton.isVisible({ timeout: 1500 }).catch(() => false)) {
        closed = await tryClickClose(overlayCloseButton).catch(() => false);
      }
      if (!closed && await backdrop.isVisible({ timeout: 1500 }).catch(() => false)) {
        await backdrop.click({ force: true, timeout: 2000 }).catch(() => {});
        await this.page.waitForTimeout(200);
        closed = !(await overlayCloseButton.isVisible().catch(() => false));
      }
      if (!closed) {
        await this.page.keyboard.press('Escape').catch(() => {});
        await this.page.waitForTimeout(150);
        await this.page.keyboard.press('Escape').catch(() => {});
        console.log('      ✓ Closed Preview Report using Escape key');
      }
      await modalWrap.waitFor({ state: 'hidden', timeout: 2000 }).catch(() => {});
      await this.page.waitForTimeout(300);
    } catch (error) {
      console.log(`      ⚠ Could not close Preview Report: ${error instanceof Error ? error.message : error}`);
    }
  }

  /**
   * Add a single section by clicking Section button and filling section name
   */
  async addSingleSection(sectionName) {
    try {
      console.log(`      - Clicking Section button to add section: ${sectionName}`);

      // Ensure overlays are cleared before interacting
      await this.ensurePreviewClosed();
      await this.page.locator('.ngx-overlay.loading-foreground').waitFor({ state: 'hidden', timeout: 5000 }).catch(() => {});

      // Capture current section input count to detect new one
      const sectionInputsAll = this.page.locator("//input[@class = 'ant-input section-name-input font-color-black ng-pristine ng-valid ng-touched']")
        .or(this.page.locator("input.section-name-input"))
        .or(this.page.locator("input[placeholder*='Section name' i]"));
      const beforeCount = await sectionInputsAll.count().catch(() => 0);

      // Use provided locator first
      const sectionButton = this.page
        .locator('//button[@class="ant-btn addSectionBtn ant-btn-primary ant-btn-background-ghost"]')
        .first()
        .or(this.page.getByRole('button', { name: /Section/i, exact: false }))
        .or(this.page.locator('button:has-text("Section")'))
        .first();

      const sectionBtnVisible = await sectionButton.isVisible({ timeout: 5000 }).catch(() => false);
      if (sectionBtnVisible) {
        await sectionButton.waitFor({ state: 'visible', timeout: 5000 });
        await sectionButton.scrollIntoViewIfNeeded();
        await sectionButton.evaluate(el => el.scrollIntoView({ block: 'center', inline: 'center' }));
        await this.page.waitForTimeout(300);
        try {
          await sectionButton.click({ timeout: 5000 });
        } catch {
          await sectionButton.click({ force: true, timeout: 5000 });
        }
      } else {
        console.log('      ⚠ Section button not found');
      }

      // Wait for a new section input to appear, then pick the last one (newly added)
      await this.page.waitForTimeout(500);
      await this.page.waitForFunction(
        (before) => {
          const inputs = document.querySelectorAll("input.section-name-input, input.ant-input.section-name-input.font-color-black.ng-pristine.ng-valid.ng-touched, input[placeholder*='Section name']");
          return inputs.length > before;
        },
        beforeCount,
        { timeout: 5000 }
      ).catch(() => {});

      const afterCount = await sectionInputsAll.count().catch(() => beforeCount);
      const sectionInput = afterCount > 0 ? sectionInputsAll.nth(afterCount - 1) : sectionInputsAll.last();
      ...
      console.log(`      ⚠ Could not add section "${sectionName}": ${error instanceof Error ? error.message : error}`);
    }
  }
}
