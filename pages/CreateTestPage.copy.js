...
class CreateTestPage {
  ...
  async clickPublish() {
    ...
    // Extra guard: ensure we are scrolled near the bottom so the button can render
    try {
      await this.page.mouse.wheel(0, 20000);
      await this.page.waitForTimeout(1000);
    } catch {
      // ignore if wheel fails (e.g., mobile emulation)
    }
    
    const publishWaitTimeout = 30000;
    ...
    await publishButton.waitFor({ state: 'visible', timeout: publishWaitTimeout });
    ...
    if (!isEnabled) {
      ...
      await expect(publishButton).toBeEnabled({ timeout: publishWaitTimeout });
    }
  ...
  async addQuestion(questionText, options, correctOptionIndex) {
    ...
    for (let i = 0; i < options.length; i++) {
      ...
      if (i < currentOptionCount) {
        ...
        // Check page state before waitFor
        if (!(await this.isPageOpen())) {
          throw new Error('Page was closed before clicking "Add an option" button');
        }
      ...
      // Check page state before waitFor
      if (!(await this.isPageOpen())) {
        throw new Error('Page was closed before filling option input');
      }
    ...
    if (correctOptionIndex < 0 || correctOptionIndex >= options.length) {
      throw new Error(
        `Invalid correctOptionIndex: ${correctOptionIndex}. ` +
        `Question has ${options.length} option(s), so correctOptionIndex must be between 0 and ${options.length - 1}.`
      );
    }
    ...
    if (!answerLetter) {
      throw new Error(`Invalid correctOptionIndex: ${correctOptionIndex}. Must be 0-${answerLetters.length - 1}.`);
    }
    ...
    // Wait for answer container to be visible
    await answerContainer.waitFor({ state: 'visible', timeout: 10000 });
    
    // Validate that the requested answer option actually exists
    // Check which answer letters are available in the DOM
    const availableAnswers = await answerContainer.locator('generic').all();
    const availableLetters = [];
    for (const answer of availableAnswers) {
      const text = await answer.textContent();
      if (text && /^[A-Z]$/.test(text.trim())) {
        availableLetters.push(text.trim());
      }
    }
    
    // If we couldn't find answers by generic locator, try a different approach
    if (availableLetters.length === 0) {
      // Try to find all answer buttons by checking for single-letter text
      const allAnswerElements = answerContainer.locator('*');
      const count = await allAnswerElements.count();
      for (let i = 0; i < count; i++) {
        const text = await allAnswerElements.nth(i).textContent();
        if (text && /^[A-Z]$/.test(text.trim())) {
          availableLetters.push(text.trim());
        }
      }
    ...
    // Validate that the requested answer exists
    if (!availableLetters.includes(answerLetter)) {
      const availableStr = availableLetters.length > 0 
        ? availableLetters.join(', ') 
        : 'none found';
      throw new Error(
        `Answer option "${answerLetter}" (index ${correctOptionIndex}) does not exist. ` +
        `Available options: ${availableStr}. ` +
        `Question has ${options.length} options, but correctOptionIndex is ${correctOptionIndex}. ` +
        `Please ensure correctOptionIndex is between 0 and ${options.length - 1}.`
      );
    }
  ...
  async isPageOpen() {
    try {
      // Try a lightweight operation to verify page is accessible
      await this.page.evaluate(() => document.readyState);
      return true;
    } catch (error) {
      // Page is closed or not accessible
      return false;
    }
  }
  ...
  async addMultipleQuestions(questionsArray) {
    ...
    for (let i = 0; i < questionsArray.length; i++) {
      ...
      if (!(await this.isPageOpen())) {
        throw new Error(`Page was closed before adding question ${i + 1}. Cannot continue.`);
      }
      ...
      while (retries > 0) {
        try {
          ...
          const isPageClosed = errorMessage.includes('Target page, context or browser has been closed') || 
                               errorMessage.includes('page has been closed') ||
                               errorMessage.includes('waitForTimeout: Target page');
          
          if (isPageClosed) {
            // Verify page is actually closed
            if (!(await this.isPageOpen())) {
              throw new Error(`Page was closed while adding question ${i + 1}. Cannot recover.`);
            }
            console.warn(`⚠ Page closure detected during question ${i + 1}. Retries remaining: ${retries - 1}`);
          }
          
          retries--;
          
          if (retries > 0) {
            // Check page is open before waiting
            if (!(await this.isPageOpen())) {
              throw new Error(`Page was closed before retry for question ${i + 1}. Cannot continue.`);
            }
            
            console.log(`Retrying question ${i + 1}... (${retries} retries remaining)`);
            await this.page.waitForTimeout(2000); // Wait before retry
          } else {
            console.error(`❌ Failed to add question ${i + 1} after 3 attempts:`, errorMessage);
            throw new Error(`Failed to add question ${i + 1}: ${errorMessage}`);
          }
        }
      }
      ...
      if (i < questionsArray.length - 1) {
        // Check page is open before waiting
        if (!(await this.isPageOpen())) {
          throw new Error(`Page was closed after adding question ${i + 1}. Cannot continue.`);
        }
        await this.page.waitForTimeout(1000);
      }
    }
  ...
  async generateAIReport() {
    ...
    let generateReportCheckbox = null;
    let checkboxFound = false;
    
    // Strategy 1: Find by text and locate nearby checkbox
    try {
      const reportText = this.page.getByText('Generate AI-Based Assessment Report', { exact: false });
      await reportText.waitFor({ state: 'visible', timeout: 5000 });
      generateReportCheckbox = reportText.locator('xpath=preceding::input[@type="checkbox"][1]')
        .or(reportText.locator('xpath=following::input[@type="checkbox"][1]'))
        .or(reportText.locator('xpath=ancestor::*//input[@type="checkbox"][1]'));
      await generateReportCheckbox.waitFor({ state: 'visible', timeout: 3000 });
      checkboxFound = true;
      console.log('✓ Found checkbox using text-based strategy');
    } catch (e) {
      console.log('⚠ Text-based strategy failed, trying alternative...');
    }
    
    // Strategy 2: Find all checkboxes and filter by context
    if (!checkboxFound) {
      try {
        ...
        for (let i = 0; i < checkboxCount; i++) {
          ...
          if (isVisible) {
            // Check if this checkbox is near the "Generate AI-Based Assessment Report" text
            const nearbyText = await checkbox.evaluate((el) => {
              const text = el.closest('*')?.textContent || '';
              return text.includes('Generate AI-Based Assessment Report') || 
                     text.includes('AI-Based Assessment Report') ||
                     text.includes('Assessment Report');
            }).catch(() => false);
            
            if (nearbyText) {
              generateReportCheckbox = checkbox;
              checkboxFound = true;
              console.log(`✓ Found checkbox at index ${i} using context-based strategy`);
              break;
            }
          }
        }
      } catch (e) {
        console.log('⚠ Context-based strategy failed');
      }
    }
    
    // Strategy 3: Try to find by label or aria-label
    if (!checkboxFound) {
      try {
        generateReportCheckbox = this.page.locator('input[type="checkbox"]')
          .filter({ hasText: /AI-Based Assessment Report/i })
          .or(this.page.locator('input[type="checkbox"][aria-label*="AI-Based Assessment Report" i]'))
          .or(this.page.locator('label:has-text("Generate AI-Based Assessment Report") input[type="checkbox"]'));
        await generateReportCheckbox.first().waitFor({ state: 'visible', timeout: 3000 });
        generateReportCheckbox = generateReportCheckbox.first();
        checkboxFound = true;
        console.log('✓ Found checkbox using label-based strategy');
      } catch (e) {
        console.log('⚠ Label-based strategy failed');
      }
    }
    
    if (!checkboxFound || !generateReportCheckbox) {
      console.log('⚠ Could not find "Generate AI-Based Assessment Report" checkbox, skipping this step');
      return;
    }
    
    // Wait for checkbox to be visible and interact with it
    await generateReportCheckbox.waitFor({ state: 'visible', timeout: 5000 });
