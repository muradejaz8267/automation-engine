...
class CreateCourse {
  ...
  async addAdditionalQuizTopic() {
    ...
    // Playwright quiz questions (6 questions with 2 options each)
    const playwrightQuizQuestions = [
      ...
      {
        questionText: 'Which Playwright method is used to navigate to a page?',
        options: [
          'page.goto()',
          'page.load()'
        ],
      ...
      {
        questionText: 'What is the purpose of `await` in Playwright?',
        options: [
          'To wait for asynchronous operations to complete',
          'To make a page reload instantly'
        ],
        correctOptionIndex: 0
      },
      {
        questionText: 'Which browser can Playwright automate?',
        options: [
      ...
      {
        questionText: 'How do you select an element with Playwright?',
        options: [
          'Using locators like `page.locator(\'selector\')`',
          'By opening DevTools manually'
        ],
        correctOptionIndex: 0
      }
    ];
  ...
  async handlePreviewReportAfterQuiz() {
    ...
    try {
      ...
      if (previewButtonVisible) {
        ...
        console.log('      ✓ Preview Report loaded and displayed successfully');
        
        // Close the preview report
        console.log('      - Closing Preview Report...');
        
        // CRITICAL: Wait for loading overlay to disappear before attempting to close
        ...
        // Try to find and click close button (X button or Close button)
        // Look specifically for Close button in the modal/dialog, not other close buttons
        const closeButton = this.page.locator('button:has-text("Close")')
          .or(this.page.getByRole('button', { name: /^Close$/i, exact: true }))
          .or(this.page.locator('nz-modal-container button:has-text("Close")'))
          .or(this.page.locator('.cdk-overlay-container button:has-text("Close")'))
        ...
        if (closeButtonVisible) {
          // Wait for button to be stable and not blocked
          ...
          // Try pressing Escape key to close modal/dialog
          await this.page.keyboard.press('Escape');
          console.log('      ✓ Closed Preview Report using Escape key');
          await this.page.waitForTimeout(1000);
        }
        
        // Ensure modal/overlay is fully closed before proceeding
        await this.ensurePreviewClosed();
        
        // CRITICAL: Click Save button immediately after closing Preview Report
        console.log('      - Clicking Save button after closing Preview Report...');
        await this.clickSaveAfterPreviewReport();
      } else {
  ...
  async addTopicWithQuizDetails(sectionContainer, topicTitle, questions, quizDetails) {
    ...
    if (quizDetails && quizDetails.enableAIReport) {
      ...
      if (checkboxVisible) {
        ...
        if (textareaVisible) {
          ...
          // Step 5.7: Click "Preview Report" button and wait for report to fully load
          console.log('      - Clicking Preview Report button...');
          const previewReportButton = this.page.getByRole('button', { name: 'Preview Report', exact: false })
            .or(this.page.locator('button:has-text("Preview Report")'))
            .first();
          ...
          try {
            ...
            console.log('      ⚠ Normal click failed, trying force click...');
            await previewReportButton.click({ force: true, timeout: 10000 });
            console.log('      ✓ Force clicked "Preview Report" button');
          }
          
          // Wait for page to fully load (use 'load' instead of 'networkidle' for better reliability)
          await this.page.waitForLoadState('load', { timeout: 30000 });
          await this.page.waitForLoadState('domcontentloaded', { timeout: 10000 });
          
          // Wait for the report to fully load and display
          console.log('      - Waiting for report to fully load and display...');
          await this.page.getByText('Assessment Report', { exact: false }).waitFor({ state: 'visible', timeout: 20000 });
          ...
          const overlayContainer = this.page.locator('.cdk-overlay-container');
          const backdrop = this.page.locator('.cdk-overlay-backdrop');
          await overlayContainer.waitFor({ state: 'hidden', timeout: 5000 }).catch(() => {});
          ...
          // After Preview Report is clicked and report is shown, stop adding more topics
          throw new Error('PREVIEW_REPORT_CLICKED_STOP_TOPICS'); // Signal to stop adding topics
        } else {
    ...
    // Step 6: Click "Save" button after adding quiz questions
    // (Only reached if Preview Report was not clicked)
    console.log(`      - Clicking Save button for topic: ${topicTitle}`);
    const saveButton = topicContainer
      .locator('button:has-text("Save")')
      .or(this.page.locator('button:has-text("Save"):not(:has-text("Save as"))'))
