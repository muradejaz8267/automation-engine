// premiumCoursePurchase.spec.js - Premium course purchase flow
// 1: Signup (new user)
// 2: Go to home page
// 3: Click on Photoshop course from home page
// 4: Go to payment-method URL
// 5-11: Fill remaining payment form fields
...
const PAYMENT_METHOD_URL = 'https://staging.fastlearner.ai/payment-method?courseId=141&courseUrl=photo-shop-mastering&price=50&premium=true';
...
test.describe('Premium Course Purchase', () => {
  test.setTimeout(180000);

  test('signup, go to home page, then purchase premium course', async ({ page, context }) => {
    // Step 1: Signup
    ...
    await delay(2000);
    console.log('✓ Step 2: Home page loaded');

    // Step 3: Click on Photoshop course from home page
    console.log('Step 3: Click on Photoshop course from home page...');
    const photoshopCourse = page.locator('a[href*="photo-shop-mastering"], a[href*="photo-shop"], a[href*="photoshop"]');
    const count = await photoshopCourse.count();
    if (count > 0) {
      await photoshopCourse.first().scrollIntoViewIfNeeded();
      await delay(500);
      await photoshopCourse.first().click({ timeout: 10000 });
    } else {
      // Fallback: navigate directly to Photoshop course if not found on home
      await page.goto('https://staging.fastlearner.ai/student/course-details/photo-shop-mastering', { waitUntil: 'domcontentloaded' });
    }

    await expect(page).toHaveURL(/course-details/);
    await delay(3000);
    console.log('✓ Step 3: Photoshop course opened');

    // Step 4: Go to payment-method URL
    console.log('Step 4: Navigating to payment-method page...');
    await page.goto(PAYMENT_METHOD_URL, { waitUntil: 'domcontentloaded', timeout: 20000 });
    await expect(page).toHaveURL(/payment-method/);
    await delay(2000);
    console.log('✓ Step 4: Payment-method page opened');

    // Step 5: Enter "Test" in First Name field
    console.log('Step 5: Entering Test in First Name field...');
    ...
    await delay(500);
    console.log('✓ Step 5: First Name field filled');
    ...
    await delay(5000); // Wait 5 seconds after Pay Now before closing browser
