// annualPremium.spec.js - Annual Premium Instructor Plan flow
// Same flow as annualStandardSubscription but clicks Get Started for Premium Instructor Plan
const { test, expect } = require('../fixtures/screenshotFixture');

const SIGN_IN_URL = 'https://staging.fastlearner.ai/auth/sign-in';
const PRICING_URL = 'https://staging.fastlearner.ai/pricing';
...
test.describe('Annual Premium', () => {
  test.setTimeout(180000);

  test('signup, click Premium Instructor Plan Get Started, fill payment form', async ({ page, context }) => {
    // Step 1: Signup
    ...
    // Step 2: Navigate to pricing page via Pricing link (stay on pricing, avoid redirect to home)
    ...
    const getStartedBtn = page.locator("xpath=//h3[contains(.,'Premium Instructor Plan')]/ancestor::div[contains(@class,'content-header')]//span[normalize-space()='Get Started']");
    await getStartedBtn.waitFor({ state: 'visible', timeout: 20000 });
    await delay(3000);
    await getStartedBtn.scrollIntoViewIfNeeded();
    await delay(500);
    await getStartedBtn.click();
    console.log('✓ Step 2: Clicked Get Started for Premium Instructor Plan');

    // Step 3: Wait for payment page to load
    ...
    await delay(5000); // Wait 5 seconds after payment

    console.log('✓ All steps completed successfully');

    await delay(5000); // Wait 5 seconds before closing browser
  });
});
