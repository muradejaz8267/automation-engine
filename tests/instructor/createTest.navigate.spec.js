...
test.describe('Navigate to Test Edit Page', () => {
  test('Login and navigate to test edit page', async ({ page }) => {
    ...
    // Fill Section 2 details - use .last() to get the most recent section (Section 2)
    console.log('Step 7: Filling Section 2 details...');
    
    // Section Name: "Section 2"
    console.log('Filling Section Name field for Section 2...');
    const sectionNameInputs = page.getByPlaceholder('Section name *').or(page.locator('input[placeholder*="Section name"]'));
    const section2NameInput = sectionNameInputs.last(); // Get the last one (Section 2)
    ...
    await section2NameInput.fill('Section 2');
    await expect(section2NameInput).toHaveValue('Section 2');
    console.log('✓ Section Name filled: Section 2');
    
    // Topic Name: "Dummy USMLE"
    console.log('Filling Topic Name field for Section 2...');
    const topicNameInputs = page.getByPlaceholder('Topic name *').or(page.locator('input[placeholder*="Topic name"]'));
    const section2TopicInput = topicNameInputs.last(); // Get the last one (Section 2)
    ...
    await section2TopicInput.fill('Dummy USMLE');
    await expect(section2TopicInput).toHaveValue('Dummy USMLE');
    console.log('✓ Topic Name filled: Dummy USMLE');
    
    // Type dropdown: Choose "Basic Quiz" (for Section 2)
    console.log('Selecting Type dropdown for Section 2: Basic Quiz...');
    await page.waitForTimeout(500);
    
    // Find the Type dropdown for Section 2 - it appears right after the Topic name input
    ...
    if (!dropdownVisible) {
      ...
      if (topicDropdownVisible) {
        ...
        // Last resort: count all dropdowns and use the one that should be for Section 2
        // Section 1 has its dropdown, Section 2 should have the next one
        const allDropdowns = page.locator('.ant-select-selector');
        const count = await allDropdowns.count();
        // Section 2 dropdown is likely the 2nd or later dropdown (after Section 1's)
    ...
    // Quiz Duration: 15
    console.log('Setting Quiz Duration for Section 2 to 15...');
    await page.waitForTimeout(500);
    const quizDurationLabels = page.locator('text=/Quiz duration/i');
    const section2QuizDurationLabel = quizDurationLabels.last(); // Get the last one (Section 2)
    ...
    console.log('Setting Passing Criteria for Section 2 to 20...');
    ...
    console.log('✓ Section 2 details filled successfully');
    
    // Step 8: Parse and add questions from USMLE file
    console.log('\nStep 8: Parsing questions from USMLE file...');
    // Get the workspace root (go up from tests/instructor to fastlearner-automation)
    const workspaceRoot = path.resolve(__dirname, '../..');
    const questionsFilePath = path.join(workspaceRoot, 'USMLE AI Generated Questions2.txt');
    ...
    console.log(`\nStep 9: Adding ${quizQuestions.length} questions to Section 2...`);
    
    // Find Section 2 container using the visible "Section 2" header button (more reliable than internal tags)
    const section2Header = page
      .getByRole('button', { name: /Section 2 Section 2 Add new section Free Student Access/i })
      .first();
    await section2Header.waitFor({ state: 'visible', timeout: 15000 });
    
    // Use the header's ancestor container as the Section 2 panel
    const section2Panel = section2Header.locator(
      'xpath=ancestor::*[contains(@class, "section") or contains(@class, "ant-collapse-item") or contains(@class, "ant-collapse-panel")][1]'
    );
    await section2Panel.waitFor({ state: 'visible', timeout: 15000 });
    ...
    for (let i = 0; i < quizQuestions.length; i++) {
      ...
      try {
        if (i === 0) {
          // First question: use Q1 slot inside Section 2
          console.log('Adding Question 1 to Q1 tab in Section 2...');
          
          const q1Input = section2Panel.getByPlaceholder('Let\'s ask a question').first();
          ...
          // Subsequent questions: add inside Section 2 panel after Q1
          const addQuestionBtn = section2Panel.getByText('Add a question', { exact: false })
            .or(section2Panel.locator('button:has-text("Add a question")'))
            .last();
          ...
          // New question input at the bottom of Section 2
          const questionInput = section2Panel.getByPlaceholder('Let\'s ask a question').last();
    ...
    console.log(`\n✓ All ${quizQuestions.length} questions added to Section 2`);
