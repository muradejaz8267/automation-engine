...
test.describe('Navigate to Test Edit Page', () => {
  test('Login and navigate to test edit page', async ({ page }) => {
    ...
    // Fill Section 4 details - use .last() to get the most recent section (Section 4)
    console.log('Step 7: Filling Section 4 details...');
    
    // Section Name: "Section 4"
    console.log('Filling Section Name field for Section 4...');
    const sectionNameInputs = page.getByPlaceholder('Section name *').or(page.locator('input[placeholder*="Section name"]'));
    const section2NameInput = sectionNameInputs.last(); // Get the last one (Section 4)
    ...
    await section2NameInput.fill('Section 4');
    await expect(section2NameInput).toHaveValue('Section 4');
    console.log('✓ Section Name filled: Section 4');
    
    // Topic Name: for Section 4
    console.log('Filling Topic Name field for Section 4...');
    const topicNameInputs = page.getByPlaceholder('Topic name *').or(page.locator('input[placeholder*="Topic name"]'));
    const section2TopicInput = topicNameInputs.last(); // Get the last one (Section 4)
    ...
    await section2TopicInput.fill('Section 4 Immunology');
    await expect(section2TopicInput).toHaveValue('Section 4 Immunology');
    console.log('✓ Topic Name filled: Section 4 Immunology');
    
    // Type dropdown: Choose "Basic Quiz" (for Section 4)
    console.log('Selecting Type dropdown for Section 4: Basic Quiz...');
    await page.waitForTimeout(500);
    
    // Find the Type dropdown for Section 4 - it appears right after the Topic name input
    ...
    // Quiz Duration: 15
    console.log('Setting Quiz Duration for Section 4 to 15...');
    await page.waitForTimeout(500);
    const quizDurationLabels = page.locator('text=/Quiz duration/i');
    const section2QuizDurationLabel = quizDurationLabels.last(); // Get the last one (Section 4)
    ...
    console.log('Setting Passing Criteria for Section 4 to 20...');
    ...
    console.log('✓ Section 4 details filled successfully');
    
    // Step 8: Parse and add questions from Section 4 file
    console.log('\nStep 8: Parsing questions from Section 4 file...');
    // Get the workspace root (go up from tests/instructor to fastlearner-automation)
    const workspaceRoot = path.resolve(__dirname, '../..');
    const questionsFilePath = path.join(workspaceRoot, 'Section 4.txt');
    ...
    console.log(`\nStep 9: Adding ${quizQuestions.length} questions to new Section (Section 4)...`);
    ...
    for (let i = 0; i < quizQuestions.length; i++) {
      ...
      try {
        if (i === 0) {
          // First question: use the last Q1 slot currently visible (new Section 4 at bottom)
          console.log('Adding Question 1 to Q1 tab in new Section (Section 4)...');
