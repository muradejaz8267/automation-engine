...
test.describe('Navigate to Test Edit Page', () => {
  test('Login and navigate to test edit page', async ({ page }) => {
    ...
    // Fill Section 3 details - use .last() to get the most recent section (Section 3)
    console.log('Step 7: Filling Section 3 details...');
    
    // Section Name: "Section 3"
    console.log('Filling Section Name field for Section 3...');
    const sectionNameInputs = page.getByPlaceholder('Section name *').or(page.locator('input[placeholder*="Section name"]'));
    const section2NameInput = sectionNameInputs.last(); // Get the last one (Section 3)
    ...
    await section2NameInput.fill('Section 3');
    await expect(section2NameInput).toHaveValue('Section 3');
    console.log('✓ Section Name filled: Section 3');
    
    // Topic Name: for Section 3
    console.log('Filling Topic Name field for Section 3...');
    const topicNameInputs = page.getByPlaceholder('Topic name *').or(page.locator('input[placeholder*="Topic name"]'));
    const section2TopicInput = topicNameInputs.last(); // Get the last one (Section 3)
    ...
    await section2TopicInput.fill('Section 3 Immunology');
    await expect(section2TopicInput).toHaveValue('Section 3 Immunology');
    console.log('✓ Topic Name filled: Section 3 Immunology');
    
    // Type dropdown: Choose "Basic Quiz" (for Section 3)
    console.log('Selecting Type dropdown for Section 3: Basic Quiz...');
    await page.waitForTimeout(500);
    
    // Find the Type dropdown for Section 3 - it appears right after the Topic name input
    ...
    if (!dropdownVisible) {
      ...
      if (topicDropdownVisible) {
        ...
        // Last resort: count all dropdowns and use the one that should be for Section 3
        // Section 1 has its dropdown, Section 3 should have the next one
        const allDropdowns = page.locator('.ant-select-selector');
        const count = await allDropdowns.count();
        // Section 3 dropdown is likely the 2nd or later dropdown (after Section 1's)
    ...
    // Quiz Duration: 15
    console.log('Setting Quiz Duration for Section 3 to 15...');
    await page.waitForTimeout(500);
    const quizDurationLabels = page.locator('text=/Quiz duration/i');
    const section2QuizDurationLabel = quizDurationLabels.last(); // Get the last one (Section 3)
    ...
    console.log('Setting Passing Criteria for Section 3 to 20...');
    ...
    console.log('✓ Section 3 details filled successfully');
    
    // Step 8: Parse and add questions from Section 3 file
    console.log('\nStep 8: Parsing questions from Section 3 file...');
    // Get the workspace root (go up from tests/instructor to fastlearner-automation)
    const workspaceRoot = path.resolve(__dirname, '../..');
    const questionsFilePath = path.join(workspaceRoot, 'Section 3.txt');
    ...
    console.log(`\nStep 9: Adding ${quizQuestions.length} questions to new Section (Section 3)...`);
    ...
    for (let i = 0; i < quizQuestions.length; i++) {
      ...
      try {
        if (i === 0) {
          // First question: use the last Q1 slot currently visible (new Section 3 at bottom)
          console.log('Adding Question 1 to Q1 tab in new Section (Section 3)...');
          ...
          // Subsequent questions: always target the last \"Add a question\" button (Section 3 at bottom)
          ...
          // New question input at the bottom (new question in Section 3)
    ...
    console.log(`\n✓ All ${quizQuestions.length} questions added to Section 3`);
