...
test.describe('Create Test Flow (Block 5)', () => {
  test('Login, load test page 269, and upload Block 5 questions', async ({ page }) => {
    ...
    await sectionNameInput.fill('Block 5');
    ...
    await topicInput.fill('Block 5 Mixed Review');
    ...
    // Step 8: Parse Block 5 questions
    const workspaceRoot = path.resolve(__dirname, '../..');
    const block5Path = path.join(workspaceRoot, 'Block 5.txt');
    const quizQuestions = parseUsmleQuestions(block5Path);
    if (!quizQuestions.length) {
      throw new Error(`No quiz questions parsed from ${block5Path}. Please check the file format.`);
    }
    ...
    for (let idx = 0; idx < quizQuestions.length; idx++) {
      ...
      console.log(`Adding Block 5 question ${idx + 1}/${quizQuestions.length}`);
    ...
    console.log('✓ All Block 5 questions added');
    ...
    console.log('✓ Block 5 publishing confirmed');
  });
});
