...
test.describe('Create Test Flow (Block 6)', () => {
  test('Login, load test page 269, and upload Block 6 questions', async ({ page }) => {
    ...
    await sectionNameInput.fill('Block 6');
    ...
    await topicInput.fill('Block 6 Mixed Review');
    ...
    // Step 8: Parse Block 6 questions
    const workspaceRoot = path.resolve(__dirname, '../..');
    const block6Path = path.join(workspaceRoot, 'Block 6.txt');
    const quizQuestions = parseUsmleQuestions(block6Path);
    if (!quizQuestions.length) {
      throw new Error(`No quiz questions parsed from ${block6Path}. Please check the file format.`);
    }
    ...
    for (let idx = 0; idx < quizQuestions.length; idx++) {
      ...
      console.log(`Adding Block 6 question ${idx + 1}/${quizQuestions.length}`);
    ...
    console.log('✓ All Block 6 questions added');
    ...
    console.log('✓ Block 6 publishing confirmed');
  });
});
