...
test.describe('Create Test Flow (Block 8)', () => {
  test('Login, load test page 269, and upload Block 8 questions', async ({ page }) => {
    ...
    await sectionNameInput.fill('Block 8');
    ...
    await topicInput.fill('Block 8 Mixed Review');
    ...
    // Step 8: Parse Block 8 questions
    const workspaceRoot = path.resolve(__dirname, '../..');
    const block8Path = path.join(workspaceRoot, 'Block 8.txt');
    const quizQuestions = parseUsmleQuestions(block8Path);
    if (!quizQuestions.length) {
      throw new Error(`No quiz questions parsed from ${block8Path}. Please check the file format.`);
    }
    ...
    for (let idx = 0; idx < quizQuestions.length; idx++) {
      ...
      console.log(`Adding Block 8 question ${idx + 1}/${quizQuestions.length}`);
    ...
    console.log('✓ All Block 8 questions added');
    ...
    console.log('✓ Block 8 publishing confirmed');
  });
});
